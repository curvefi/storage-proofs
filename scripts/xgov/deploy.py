...
NETWORK = f"https://bsc-dataseed2.defibit.io"
...
def send_blockhash():
    ...
    sender = boa.load_partial(
        "contracts/xdao/contracts/messengers/LayerZeroSender.vy"
    ).at("0x49cdecc38B4CAf6a07c13558A32820333BC2aB61")
    sender.transmit(block_num - 64 - 3, value=sender.quote())


def simulate(relayer, verifier):
    relayer = boa.load_partial("contracts/xdao/contracts/xgov/XYZRelayer.vy").at(
        relayer
    )
