...
def hashmap(slot, value, type) -> int:
    ...
    return int(web3.Web3.keccak(eth_abi.encode([f"(uint256,{type})"], [[slot, value]])).hex(), 16)


def keccak256(value: int) -> int:
    return int(web3.Web3.keccak(eth_abi.encode([f"uint256"], [value])).hex(), 16)
...
def get_total_slots(eth_web3, block, slope_changes_num=4):
    ...
    start = WEEK + (point_history_ts := point_history[2]) // WEEK * WEEK
    return [
        3,  # epoch
        *(point_history_slot + i for i in range(4)),  # point history
        *(hashmap(7, start + i * WEEK, "uint256") for i in range(slope_changes_num)),  # slope changes
    ]
...
def generate_balance_proof(user, eth_web3, block_number=BLOCK_NUMBER, log=False):
    ...
    proofs = serialize_proofs(eth_web3.eth.get_proof(
        VOTING_ESCROW,
        total_slots + balance_slots,
        block_number,
    ), encode=False)
    proof_rlp = rlp.encode([proofs[0], proofs[1:1+len(total_slots)], proofs[1+len(total_slots):]])
...
def generate_total_proof(eth_web3, block_number=BLOCK_NUMBER, log=False):
    block = eth_web3.eth.get_block(block_number)
    if log:
        print(f"Generating proof for block {block.number}, {block.hash.hex()}")
    block_header_rlp = serialize_block(block)
...
def generate_delegation_proof(chain_id, user, eth_web3, block_number=BLOCK_NUMBER, log=False):
    ...
    proof_rlp = serialize_proofs(eth_web3.eth.get_proof(
        VECRV_DELEGATE,
