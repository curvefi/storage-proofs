...
def update_balance(
    ...
    assert self.user_point_epoch[_user] <= _user_point_epoch and \
        self.user_point_history[_user][_user_point_epoch].ts <= _user_point_history.ts, "Outdated update"
...
@external
def update_total(_epoch: uint256, _point_history: Point, _slope_changes: DynArray[int128, SLOPE_CHANGES_CNT]):
    """
    ...
    assert self.epoch <= _epoch and self.point_history[_epoch].ts <= _point_history.ts, "Outdated update"
